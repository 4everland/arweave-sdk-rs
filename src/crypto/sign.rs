...
impl Signer for EthSigner {
    fn sign(&self, message: &[u8]) -> Result<Vec<u8>, Error> {
        match &self.key {
            Some(key) => {
                ...
                let result = &result[..];
                let msg = libsecp256k1::Message::parse_slice(result)
    ...
    fn public_key(&self) -> Result<Base64, Error> {
        let key = match &self.key {
            Some(k) => k,
            _ => {
                return Err(Error::InvalidKeyError);
            }
        };
...
impl EthSigner {
    fn from_mnemonic(mnemonic: &str, passphrase: &str) -> Result<EthSigner, Error> {
        let seed = bip39::Mnemonic::parse_in_normalized(English, mnemonic)
            .map_err(|_| Error::MnemonicDecodeError)?;
        let pri_key = bitcoin::bip32::Xpriv::new_master(
            bitcoin::Network::Bitcoin,
            seed.to_seed(passphrase).as_ref(),
        )
        .map_err(|_| Error::MnemonicDecodeError)?;
        let x = pri_key.private_key;

        let key = libsecp256k1::SecretKey::parse_slice(x.secret_bytes().as_slice())
            .map_err(|_| Error::InvalidKeyError)?;
        let pub_key = libsecp256k1::PublicKey::from_secret_key(&key);
    ...
    pub fn from_prv_hex(prv: &str) -> Result<EthSigner, Error> {
        ...
        let key = libsecp256k1::SecretKey::parse_slice(decode_key.as_slice())
            .map_err(|_| Error::InvalidKeyError)?;
        let pub_key = libsecp256k1::PublicKey::from_secret_key(&key);
    ...
    fn from_address(addr: &str) -> Result<EthSigner, Error> {
        let addr =
            ethaddr::Address::from_str_checksum(addr).map_err(|_| Error::AddressDecodeError)?;
        Ok(Self {
            key: None,
            address: addr.0,
        })
    ...
    fn from_pubkey(key: &[u8]) -> Result<EthSigner, Error> {
        let key = libsecp256k1::PublicKey::parse_slice(key, None)
            .map_err(|_| Error::AddressDecodeError)?;
        let mut address = [0u8; 20];
        let mut hasher = Keccak256::new();
    ...
    fn verify_result(&self, message: &[u8], signature: &[u8]) -> Result<(), Error> {
        ...
        let rec_id =
            libsecp256k1::RecoveryId::parse(signature[0]).map_err(|_| Error::InvalidSignature)?;
        let sig = libsecp256k1::Signature::parse_standard_slice(&signature[1..])
            .map_err(|_| Error::InvalidSignature)?;
        let msg =
            libsecp256k1::Message::parse_slice(&message).map_err(|_| Error::InvalidSignature)?;
        let pubkey =
            libsecp256k1::recover(&msg, &sig, &rec_id).map_err(|_| Error::InvalidSignature)?;
        let mut address = [0u8; 20];
...
impl Signer for ArweaveSigner {
    ...
    fn public_key(&self) -> Result<Base64, Error> {
        let key = match &self.keypair {
            Some(k) => k,
            _ => {
                return Ok(Base64::from(self.n.as_slice()));
            }
        };
...
impl ArweaveSigner {
    ...
    fn verify_result(&self, message: &[u8], signature: &[u8]) -> Result<(), Error> {
        let n = match &self.keypair {
            Some(key) => key.rsa().unwrap().n().to_vec(),
            _ => self.n.to_vec(),
        };
        ...
        let pkey = PKey::from_rsa(
            Rsa::from_public_components(
                BigNum::from_slice(&n)
                    .map_err(|_| Error::InvalidSignature)
                    .unwrap(),
                BigNum::from_slice(&Base64::from_str("AQAB").unwrap().0)
                    .map_err(|_| Error::InvalidSignature)
                    .unwrap(),
            )
            .map_err(|_| Error::InvalidSignature)
        ...
        let mut verify = Verifier::new(MessageDigest::sha256(), &pkey)
            .map_err(|_| Error::InvalidSignature)
            .unwrap();
        verify
            .set_rsa_padding(Padding::PKCS1_PSS)
...
mod tests {
    ...
    fn test_default_keypair() {
        let provider =
            ArweaveSigner::from_keypair_path(DEFAULT_WALLET_PATH).expect("Valid wallet file");
        assert_eq!(
    ...
    fn test_eth_signer() -> Result<(), error::Error> {
        let signer = EthSigner::from_prv_hex(
            "1f534ac18009182c07d266fe4a7903c0bcc8a66190f0967b719b2b3974a69c2f",
        )?;
    ...
    fn test_eth_pub_key() -> Result<(), error::Error> {
        let decode_key =
            hex::decode("02D0E95DEE9217777132128ACA9CE7191EA391ACCB83FF47B5C984D8823FAD331E")
                .map_err(|_| Error::InvalidKeyError)?;
        let signer = EthSigner::from_pubkey(decode_key.as_slice())?;
        assert_eq!(
            signer.wallet_address(),
            "0x02A71bA6943D302c0152cdb237CB606ffE1C9BC4"
        );
        Ok(())
    }
    ...
    fn test_eth_verify() -> Result<(), error::Error> {
        let message =
            Base64::from_str("MFNXFMKDurrmwEZYoH99MVPg9pLodCgz5moTcBq2xtVrP1RQDgYit5WQgJ8h42BU")
                .unwrap();
